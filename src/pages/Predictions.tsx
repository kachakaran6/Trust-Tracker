...
function Predictions() {
  ...
  const predictCategoryExpenses = () => {
    ...
    const predictions = expenseCategories.map(category => {
      let totalSpent = 0;
      let monthCount = 0;
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (categoryTransactions.length > 0) {
          totalSpent += categoryTransactions.reduce((sum, t) => sum + t.amount, 0);
          monthCount++;
        }
      }
  ...
  const nextMonthPrediction = expensePredictions[expensePredictions.length - predictionRange];
  ...
  const topCategoriesWithPredictions = categoryPredictions
    .filter(category => category.prediction > 0 && typeof category.trend === 'number')
    .slice(0, 2);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 flex items-start animate-fade-in">
        ...
        <div>
          ...
          <p className="text-sm text-blue-700 mt-1">
            Our AI-powered predictions are based on your historical spending patterns and use machine learning to forecast future expenses. These predictions are estimates and may vary from actual spending based on your financial behavior.
          </p>
        </div>
      </div>
      ...
      <div className="card p-6 animate-slide-up">
        <div className="flex items-center mb-4">
          ...
          <div>
            <h2 className="text-lg font-semibold">Next Month's Projected Expenses</h2>
            <p className="text-sm text-gray-500">Based on your spending history and AI analysis</p>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mt-4">
          <div className="text-center md:text-left mb-4 md:mb-0">
            ...
            <p className="text-3xl font-bold text-purple-600">{formatCurrency(nextMonthTotal)}</p>
          </div>
      ...
      <div className="card p-6 animate-slide-up" style={{ animationDelay: '0.1s' }}>
        <h2 className="text-lg font-semibold mb-4">Expense Trend & Prediction</h2>
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <ReferenceLine x={expensePredictions[5].month} stroke="#777" strokeDasharray="3 3" label={{ value: 'Prediction starts', position: 'top' }} />
              <Legend />
              <Line 
                type="monotone" 
      ...
      <div className="card p-6 animate-slide-up" style={{ animationDelay: '0.2s' }}>
        <h2 className="text-lg font-semibold mb-4">Predicted Expenses By Category</h2>
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
            ...
            >
              ...
              <Bar dataKey="prediction" name="Predicted Amount" fill="#8B5CF6" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
            ...
            <ul className="space-y-2 text-sm">
              {topCategoriesWithPredictions.map(category => (
                <li key={category.id} className="flex items-start">
                  ...
                  <span>Your <strong>{category.name}</strong> spending is predicted to {category.trend > 0 ? 'increase' : 'decrease'} by {Math.abs(category.trend)}%. Consider setting a budget limit.</span>
                </li>
              ))}
              {nextMonthTotal > 0 && (
                <li className="flex items-start">
                  <div className="w-4 h-4 rounded-full bg-amber-100 mt-0.5 mr-2"></div>
                  <span>Based on your income, aim to keep your total monthly expenses below {formatCurrency(nextMonthTotal * 0.9)}.</span>
                </li>
              )}
            </ul>
          </div>
            ...
            <ul className="space-y-2 text-sm">
              {topCategoriesWithPredictions[0]?.prediction > 0 && (
                <li className="flex items-start">
                  ...
                  <span>If you reduce your top category spending by 15%, you could save approximately {formatCurrency(topCategoriesWithPredictions[0].prediction * 0.15)} next month.</span>
                </li>
              )}
              <li className="flex items-start">
                <div className="w-4 h-4 rounded-full bg-green-100 mt-0.5 mr-2"></div>
                <span>Setting up automatic transfers to a savings account can help you save consistently.</span>
              </li>
              <li className="flex items-start">
                <div className="w-4 h-4 rounded-full bg-blue-100 mt-0.5 mr-2"></div>
                <span>Consider allocating 20% of your income to savings each month.</span>
              </li>
            </ul>
          </div>
        </div>
