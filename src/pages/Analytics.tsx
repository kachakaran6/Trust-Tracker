      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="card p-6 animate-fade-in">
          <div className="text-center">
            ...
            <p className="text-2xl font-bold text-success-600">{formatCurrency(monthlySummary.totalIncome)}</p>
          </div>
        </div>
        ...
        <div className="card p-6 animate-fade-in" style={{ animationDelay: '0.1s' }}>
          <div className="text-center">
            ...
            <p className="text-2xl font-bold text-danger-600">{formatCurrency(monthlySummary.totalExpense)}</p>
          </div>
        </div>
        ...
        <div className="card p-6 animate-fade-in" style={{ animationDelay: '0.2s' }}>
          <div className="text-center">
            ...
            <p className={`text-2xl font-bold ${
              monthlySummary.balance >= 0 ? 'text-primary-600' : 'text-danger-600'
            }`}>
      ...
      <div className="card p-6 animate-slide-up" style={{ animationDelay: '0.3s' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-green-50 p-4 rounded-lg border border-green-100">
            ...
            <p className="text-sm">
              {monthlySummary.totalExpense > (yearlyTrendData[yearlyTrendData.length - 2]?.expenses || 0)
                ? `Your spending increased by ${formatCurrency(monthlySummary.totalExpense - (yearlyTrendData[yearlyTrendData.length - 2]?.expenses || 0))} compared to last month.`
                : `Your spending decreased by ${formatCurrency((yearlyTrendData[yearlyTrendData.length - 2]?.expenses || 0) - monthlySummary.totalExpense)} compared to last month.`
              }
            </p>
            <div className="mt-2">
              ...
              <div className="flex items-center">
                <div className={`h-2 rounded-full ${
                  monthlySummary.totalExpense > (yearlyTrendData[yearlyTrendData.length - 2]?.expenses || 0) 
                    ? 'bg-danger-500' : 'bg-success-500'
                }`} style={{ 
                  width: `${Math.min(Math.abs(monthlySummary.totalExpense - (yearlyTrendData[yearlyTrendData.length - 2]?.expenses || 0)) / monthlySummary.totalExpense * 100, 100)}%` 
                }}></div>
              </div>
            </div>
          </div>
